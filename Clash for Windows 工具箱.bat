...
echo 当前内核: !CURTYPE!
echo.
for %%A in ("1. 切换内核" "2. 更新内核" "3. 控制 Clash" "4. 整理内核文件" "5. 查看内核应用" "6. 注入分流规则" "7. 更新 Geo 数据库" "8. 退出") do echo %%~A
echo.
set /p "CHOICE=选择 [1-8]:"
if "%CHOICE%"=="1" (call :sub_switch & goto menu)
if "%CHOICE%"=="2" (call :sub_update & goto menu)
if "%CHOICE%"=="3" (call :sub_clash_control & goto menu)
if "%CHOICE%"=="4" (call :tidy 2>nul & goto menu)
if "%CHOICE%"=="5" (call :show_detail 2>nul & goto menu)
if "%CHOICE%"=="6" (call :inject_mixin & goto menu)
if "%CHOICE%"=="7" (call :update_geo & goto menu)
